...
However, indexes are not free—**every index speeds up READS but slows down WRITES (`INSERT`, `UPDATE`, `DELETE`)** because the database must keep both the table files and the B-Tree index files perfectly in sync.

When our tables scale past **10,000+ rows**, we will deploy this targeted indexing blueprint:
...
Execute these commands when performance flags appear in production APM tools (Sentry/Datadog):
